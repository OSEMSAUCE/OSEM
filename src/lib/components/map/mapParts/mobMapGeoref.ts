...
async function tryInflate(data: Uint8Array): Promise<Uint8Array | null> {
    for (const format of ["deflate", "deflate-raw"] as const) {
        try {
            ...
            const output = readable.pipeThrough(
...
async function getDecompressedStreams(buffer: ArrayBuffer): Promise<string[]> {
    try {
        ...
        while ((m = streamRe.exec(rawText)) !== null) {
            ...
            checked++;
            const streamBytes = rawBytes.slice(
                dataStart,
                dataStart + streamLen,
            );
...
function extractWktProjection(text: string): string | null {
    for (const token of ["PROJCS", "GEOGCS"]) {
        ...
        for (
            ...
            i < idx + token.length + 5 && i < text.length;
            i++
        ) {
            if (text[i] === "[") {
                openChar = "[";
...
function affinePairConverters(
    ...
    if (canvasPts.length < 3 || gpsPts.length < 3)
...
function orientationChecked(
    ...
    console.log(
        "[georef] GPTS orientation fix — yFlip:",
        yFlip,
        "xFlip:",
        xFlip,
    );
...
function lptsToCanvas(
    ...
    switch (((rotation % 360) + 360) % 360) {
        case 90:
            return [ly * cw, lx * ch];
        case 180:
            return [(1 - lx) * cw, ly * ch];
        case 270:
            return [(1 - ly) * cw, (1 - lx) * ch];
        default:
            return [lx * cw, (1 - ly) * ch]; // 0°
    }
}
...
function extractGptsFromText(
    ...
    for (let i = 0; i + 1 < gptsNums.length; i += 2)
        gpsPts.push([gptsNums[i], gptsNums[i + 1]]);

    // Validate GPS values are reasonable (lat ±90, lon ±180)
    if (gpsPts.some(([lat, lon]) => Math.abs(lat) > 90 || Math.abs(lon) > 180))
        return null;
    ...
    if (lptsMatch) {
        ...
        if (lptsNums.length >= gpsPts.length * 2) {
            ...
            console.log(
                "[georef] GPTS: LPTS positions computed, rotation=",
                rotation,
            );
        } else {
...
function findLgiCtm(texts: string[]): LgiCtm | null {
    ...
    if (!projcs) {
        ...
        for (const text of texts) {
            ...
            if (idx !== -1) {
                console.log(
                    "[georef] B: LGIDict context:",
                    text.slice(idx, idx + 500).replace(/[^\x20-\x7e\n]/g, "?"),
                );
            }
        }
        return null;
...
function extractGdalGeoref(text: string): GdalGeoref | null {
    ...
    const gt = tMatch.slice(1, 7).map(Number) as [
        number,
        number,
        number,
        number,
...
function searchXmpMetadata(xmpRaw: string): GptsBounds | null {
    const n = xmpRaw.match(
        /(?:north|maxLat|NorthBoundingCoordinate)[^>]*?>([\d.-]+)</i,
    );
    const s = xmpRaw.match(
        /(?:south|minLat|SouthBoundingCoordinate)[^>]*?>([\d.-]+)</i,
    );
    const e = xmpRaw.match(
        /(?:east|maxLon|EastBoundingCoordinate)[^>]*?>([\d.-]+)</i,
    );
    const w = xmpRaw.match(
        /(?:west|minLon|WestBoundingCoordinate)[^>]*?>([\d.-]+)</i,
    );
...
async function extractGeorefImpl(
    ...
    const renderScale = Math.min(
        2.0,
        2048 / Math.max(rawViewport.width, rawViewport.height),
    );
    ...
    console.log(
        "[georef] rendered page at scale",
        renderScale,
        `→ ${canvasWidth}×${canvasHeight}, page ${pageWidthPts.toFixed(1)}×${pageHeightPts.toFixed(1)} pts`,
    );
    ...
    console.log(
        `[georef] searching ${streams.length} decompressed streams + raw text`,
    );
    ...
    for (let i = 0; i < streams.length; i++) {
        ...
        const gptsStream = extractGptsFromText(
            text,
            canvasWidth,
            canvasHeight,
            rotation,
        ...
        if (gdalStream) {
            console.log(
                `[georef] C: GDAL affine found in stream ${i}`,
                gdalStream.gt,
            );
    ...
    if (lgi) {
        console.log(
            "[georef] B: LGIDict CTM found across all sources, det=",
            lgi.a * lgi.d - lgi.b * lgi.c,
        );
    ...
    try {
        ...
        const rawMeta = meta.metadata as unknown as {
            getRaw?: () => string;
        } | null;
        const xmpRaw =
            rawMeta && typeof rawMeta.getRaw === "function"
                ? rawMeta.getRaw()
                : "";
        if (xmpRaw) {
    ...
    const hasProj = allTexts.some(
        (t) => t.includes("PROJCS") || t.includes("GEOGCS"),
    );
    const hasGpts = allTexts.some((t) => t.includes("/GPTS"));
    console.warn(
        "[georef] no georeference found. Tokens found — /LGIDict:",
        hasLgi,
        "/CTM:",
        hasCtm,
        "PROJCS/GEOGCS:",
