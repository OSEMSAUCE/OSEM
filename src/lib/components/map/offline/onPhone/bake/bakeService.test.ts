...
vi.mock("$osem/components/map/offline/r2Worker/roads/packDownload", () => ({
...
vi.mock("$osem/components/map/offline/onPhone/store/tombstones/purgeRoadRasters", () => ({
...
vi.mock("$osem/components/map/offline/onPhone/offlineDownloadGate", () => ({
...
vi.mock("$osem/components/map/offline/onPhone/satellite/satelliteImage", () => ({
...
vi.mock("$osem/components/map/offline/onPhone/store/tombstones/legacyVectorCleanup", () => ({
	deleteVectorAt: h.deleteVectorAt,
	getVectorKeys: vi.fn(async () => []),
	getVectorFeaturesAt: vi.fn(async () => []),
}));

vi.mock("$osem/components/map/offline/onPhone/store/coverageRegistry", () => ({
