...
<Card.Root class="my-4">
	<Card.Content class="pt-6 pb-4">
		...
		<div class="text-center mb-6">
			<p class="text-sm sm:text-2xl font-bold text-muted-foreground mb-2 tracking-wider">
				DATA COMPLETION
			</p>
			{#if dataCompletion != null}
				<p class="text-5xl sm:text-8xl py-2 pb-4 font-bold tabular-nums leading-none">
					{dataCompletion}%
				</p>
			{:else}
				<p class="text-6xl py-2 pb-4 font-bold text-muted-foreground/20 leading-none">
					—
				</p>
			{/if}
		</div>
		...
		{#if showSecondaryStats}
			<div class="grid {gridColsClass} gap-3 pt-4 border-t border-border/40 text-center">
				...
				{#if fieldPointsScored != null && fieldPointsAvail != null}
					<div>
						<p class="font-semibold">{fieldPointsScored} / {fieldPointsAvail}</p>
		...
		<p class="text-xs text-muted-foreground leading-relaxed mt-2">
			This score reflects our best efforts to find publicly available data. Missing
			data? Help us improve this score at
			<a href="mailto:info@ReTreever.org" class="text-accent hover:underline"
		...
		<details class="mt-2 pt-1">
			...
			<div class="mt-3 space-y-3">
				<div class="grid grid-cols-4 gap-1.5 text-center text-xs">
					...
					<div class="rounded-lg p-2 bg-muted/20 border border-border">
					...
					<div class="rounded-lg p-2 bg-accent/10 border border-accent/20">
				...
				<p class="text-xs text-muted-foreground leading-relaxed">
					The ReTreever Score (percentile 0–100) measures data availability across
					{scoreLabel === "ORGANIZATION SCORE" ? "organizational project" : "project"} attributes.
					Score reflects how much information is publicly accessible relative to their total
					production claims, measured against other
					{scoreLabel === "ORGANIZATION SCORE" ? "organizations" : "projects"} in the database.
					For more info see the data below, and the associated
					<a href="/what">project pages</a>. Or check out the open source repository at
					<a href="https://github.com/OSEMSAUCE/OSEM">github.com/OSEMSAUCE/OSEM🤘🌲</a>.
				</p>
			</div>
		</details>
