<script lang="ts">
	...
	const dotMatrixText = $derived(
		scoreLabel.replace(/\s+/g, " ").toUpperCase(),
	);
...
<Card.Root class="my-4">
	...
	<Card.Content class="pt-0 pb-4">
		...
		<div
			class="flex items-start divide-x divide-border mb-6 overflow-hidden"
		>
			...
			<div class="flex-1 text-center px-2 sm:px-4">
				...
				{#if percentile != null || percentile != undefined}
					<p
						class="text-5xl sm:text-8xl py-2 pb-4 font-bold text-accent tabular-nums leading-none"
					>
						{percentile}
					</p>
					{#if percentile <= 34}
		...
		<details class="mt-2 pt-1">
			...
			<div class="mt-3 space-y-3">
				...
				<p class="text-xs text-muted-foreground leading-relaxed">
					...
					<a href="http://localhost:5173/what">project pages</a>. Or
					check out the open source repository at
