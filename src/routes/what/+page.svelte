<script lang="ts">
	...
	import { createLandUrl } from "../../lib/utils/centroidUrls";
	...
	const customRenderers = $derived(() => {
		...
		// landName → link to map view using centroid coordinates
		renderers.landName = (value: unknown, row: Record<string, unknown>) => {
			if (!value) return { component: "text", props: { label: "" } };

			// Try to use centroid coordinates if available
			if (row.centroid && typeof row.centroid === "string") {
				const centroid = extractCentroidFromJson(row.centroid);
				if (centroid) {
					const mapUrl = createCentroidUrl(centroid, String(value));
					return {
						component: "link",
						props: {
							href: mapUrl,
			...
			// Fallback to old URL structure if no centroid available
			const projectId = row.projectId || urlProjectId;
			const landId = row.landId;
			if (!projectId) {
				return { component: "text", props: { label: String(value) } };
			}
			let mapUrl = `/where?project=${String(projectId)}`;
			if (landId) {
